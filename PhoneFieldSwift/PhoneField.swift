...
class PhoneField: UITextField {
    ...
    var _previousText:String!                           //保存上一次的文本内容
    var _previousRange:UITextRange!                     //保持上一次的文本范围
    ...
    func setTextFieldInit() {
        self.borderStyle = UITextBorderStyle.none
        self.keyboardType = UIKeyboardType.numberPad
        self.background = UIImage(named:"home_line_right");
    }
    ...
    override func willMove(toSuperview newSuperview: UIView?) {
        ...
        if newSuperview != nil 
        {
            NotificationCenter.default.addObserver(self,selector: #selector(phoneNumberFormat(_:)),name: NSNotification.Name.UITextFieldTextDidChange,object: nil)
        }
        else
        {
...
//        }
        ...
        //过滤掉非数字字符，只保留数字
        var digitsText = getDigitsText(string: textField.text!,cursorPosition: &cursorPostion)
    ...
    func getDigitsText(string:String, cursorPosition:inout Int) -> String{
        ...
        {
            //如果是数字则添加到返回结果中 //非数字则跳过，如果这个非法字符在光标位置之前，则光标需要向前移动
